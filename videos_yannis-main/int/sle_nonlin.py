...
def dudt(time: float,  # pylint: disable=unused-argument
    ...
    return values - (1.0 + 1.0j*c_2)*np.abs(values)**2*values - \
        (1.0+1.0j*nu)*np.mean(values) + \
        (1.0+1.0j*c_2)*np.mean(np.abs(values)**2*values)
...
def create_initial_conditions(n_oscillators: int) -> np.ndarray:
    ...
    """
    return 1.0+ 0.5 * np.random.randn(int(n_oscillators))
...
def integrate(n_oscillators: int = 256,
    ...
    pars = [0.58, 1.49, 1.02] if pars is None else pars
    c_2, nu, eta = pars
    ...
    initial_condition = initial_condition/np.mean(initial_condition)*eta
    ...
    sol = solve_ivp(dudt,
                    [0, t_eval[-1]],
                    initial_condition.astype('complex'),
                    t_eval=t_eval,
                    args=(c_2, nu))
